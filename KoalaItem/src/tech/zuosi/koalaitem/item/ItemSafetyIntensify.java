 ...
 */
public class ItemSafetyIntensify implements IItem {
    private ItemStack model = new ItemStack(Material.EMERALD);

    public ItemSafetyIntensify() {}

    @Override
    public ItemStack defaultItem() {
        return new NBTUtil(model)
                .initData("安全强化石", ItemType.ITEM,new String[]{"0","0","0"})
                .initLore(new String[]{
                        ChatColor.GREEN + "【功能】" + ChatColor.LIGHT_PURPLE + "强化武器，失败后武器不会降级",
                        ChatColor.GREEN + "【用法】" + ChatColor.RED + "与物品一起放入锻造台"
                })
                .setDisplayName(ChatColor.GREEN + "安全强化石")
                .getItemStack();
    }
    ...
    public boolean validate(ItemStack is) {
        ...
        return ((String) type).contains("ITEM") && name.equals("安全强化石");
    }
}
