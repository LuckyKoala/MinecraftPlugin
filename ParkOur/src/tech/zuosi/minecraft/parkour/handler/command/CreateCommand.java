 ...
 */
public class CreateCommand implements CommandDispatcher {
    private static final String CREATE_COMMAND_LABEL = "create";

    @Override
    public void onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if(CREATE_COMMAND_LABEL.equals(label)) {
            if(args.length == 0) {
                ...
                if(sender instanceof Player) {
                    ...
                    if(args.length == 1) {
                        ...
                        if(MapPath.EMPTY == mapPath) {
                            ...
                            if(gameManager.fromMapPath(mapPath) == null) {
                                Core.getInstance().selectionManager.selectionMap
                                        .putIfAbsent(player.getName(),
                                                new Selection(player.getName(), mapPath, Selection.Purpose.CREATE));
                                player.sendMessage(ChatColor.GREEN + "进入选取程序");
                                player.sendMessage(SelectionManager.SELECTION_USAGE);
                            } else {
                                player.sendMessage(ChatColor.RED + "指定地图已存在");
                            }
                        }
                    } else {
                        displayHelpInfo(sender);
    ...
    public void displayHelpInfo(CommandSender sender) {
        sender.sendMessage(ChatColor.RED + "Usage: " + TOP_COMMAND_LABEL + " create <"+PATH_FORMAT +">");
    }
}
